...
def get_jsonbin_data(use_cache=True):
    """JSONBin에서 데이터 가져오기 (캐싱 적용)"""
    # 캐시된 데이터가 있으면 사용
    if use_cache and '_jsonbin_cache' in st.session_state:
        return st.session_state['_jsonbin_cache']
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            st.session_state['_jsonbin_cache'] = data
            return data
    except:
        pass
    return None
...
def is_device_approved_jsonbin():
    """현재 기기가 JSONBin에서 승인되었는지 확인 (캐싱 적용)"""
    # 이미 승인된 것으로 확인되면 캐시 사용
    if st.session_state.get('_device_approved_cache'):
        return True
    ...
    if data and "approved_devices" in data:
        is_approved = device_id in data["approved_devices"]
        if is_approved:
            st.session_state['_device_approved_cache'] = True
        return is_approved
    return False
...
# JSONBin 초기화 (첫 실행 시 Bin 생성) - 백그라운드로
if not get_bin_id():
    try:
        create_jsonbin()
    except:
...
# 이미 인증된 상태면 바로 통과 (JSONBin 호출 안 함)
if saved_config.get('authenticated') and saved_config.get('password') == CORRECT_PASSWORD:
    st.session_state['authenticated'] = True
    if saved_config.get('device_id'):
...
if not st.session_state['authenticated']:
    ...
    with col2:
        ...
        if not device_id:
            ...
            if st.button("🔓 관리자로 바로 접속", key="btn_admin_first"):
                if admin_pw_first == ADMIN_PASSWORD:
                    ...
                    st.session_state['authenticated'] = True
                    save_config({'authenticated': True, 'password': CORRECT_PASSWORD, 'device_id': new_code})
        ...
        # 브라우저 ID가 있으면 승인 여부 확인
            ...
            if st.button("입장", key="btn_login", use_container_width=True, type="primary"):
                if pw == CORRECT_PASSWORD or pw == ADMIN_PASSWORD:
                    st.session_state['authenticated'] = True
                    if remember_pw:
                        save_config({
                            'authenticated': True,
            ...
            if st.button("🔄 승인 상태 확인", key="btn_check_approval"):
                # 캐시 무시하고 새로 확인
                st.session_state.pop('_device_approved_cache', None)
                st.session_state.pop('_jsonbin_cache', None)
            ...
            if st.button("🔓 관리자로 승인", key="btn_admin_approve"):
                if admin_pw == ADMIN_PASSWORD:
                    ...
                    st.session_state['authenticated'] = True
                    save_config({'authenticated': True, 'password': CORRECT_PASSWORD, 'device_id': device_id})
                    st.success("✅ 관리자 기기로 승인되었습니다!")
                    st.rerun()
                else:
                    st.error("관리자 비밀번호가 틀렸습니다.")
            ...
            if st.button("🔄 다른 코드로 변경", key="btn_change_device"):
                st.session_state.pop('browser_device_id', None)
                st.rerun()
    st.stop()
...
with st.sidebar:
    ...
    if 'claude_model' not in st.session_state:
        st.session_state['claude_model'] = "claude-opus-4-20250514"

    model_options = {
        "Claude Opus 4 (최고 품질)": "claude-opus-4-20250514",
...
def ask_ai(prompt, temp=0.7):
    ...
    model = st.session_state.get('claude_model', 'claude-opus-4-20250514')
